...
typedef Widget ErrorBuilder(BuildContext context,RetryCallback retryCallback, Object error);
typedef Widget LoadingBuilder(BuildContext context);
typedef Widget NoItemsFoundBuilder(BuildContext context);
typedef Widget RetryBuilder(BuildContext context, RetryCallback retryCallback,Object error);
...
class PagewiseState<T> extends State<Pagewise<T>> {
  ...
  Widget _itemBuilder(BuildContext context, int index) {
    ...
    if (index == total - 1) {
      ...
      if (this._effectiveController.error != null) {
        if (widget.showRetry) {
          return this._getRetryWidget(this._effectiveController.error);
        } else {
  ...
  Widget _getErrorWidget(Object error) {
    return this._getStandardContainer(
        child: widget.errorBuilder != null
            ? widget.errorBuilder(context, this._effectiveController.retry,this._effectiveController.error)
            : Text('Error: $error',
                style: TextStyle(
  ...
  Widget _getRetryWidget(Object error) {
    ...
    return this._getStandardContainer(
        child: widget.retryBuilder != null
            ? widget.retryBuilder(context, this._effectiveController.retry,error)
            : defaultRetryButton);
  }
...
///show loading in first item
///may error happend in showing network exception [ in time items width and height are small ]
...
import 'package:flutter_staggered_grid_view/flutter_staggered_grid_view.dart';
                ...
                Expanded(
                  child: PagewiseStaggeredGridView<Market>.countBuilder(
/*                    pageSize: 20,
                    pageFuture: (pageIndex) {
                      return getList(20,pageIndex);
                    },*/
                    pageLoadController: pagewiseLoadController,// for can reset results
                    crossAxisCount: 2,
                    shrinkWrap: false,
                    staggeredTileBuilder: (int index) => StaggeredTile.fit(1),//StaggeredTile.count(2, index.isEven ? 2 : 1),
                    scrollDirection: Axis.vertical,
                    reverse: false,
                    padding: EdgeInsets.all(4.0),
                    itemBuilder: (context, item, index) {
                      return MarketItem(_scaffoldKey,item,snapshot.data);
                    },
  ...
  */
  /// Creates a Pagewise StaggeredGridView with a crossAxisCount.
  ///
  /// All the properties are either those documented for normal [SliverGrid](https://docs.flutter.io/flutter/widgets/SliverGrid-class.html)
  /// or those inherited from [Pagewise]


  PagewiseStaggeredGridView(
      {Key key,
        bool addAutomaticKeepAlives: true,
        bool addRepaintBoundaries: true,
  ...
  /// All the properties are either those documented for normal [SliverGrid](https://docs.flutter.io/flutter/widgets/SliverGrid-class.html)
  /// or those inherited from [Pagewise]
  PagewiseStaggeredGridView.extent(
      {Key key,
        bool addAutomaticKeepAlives: true,
        bool addRepaintBoundaries: true,
        @required double maxCrossAxisExtent,
  ...
  PagewiseStaggeredGridView.custom(
      {Key key,
        bool primary,
        ScrollController controller,
        EdgeInsetsGeometry padding,
  ...
  PagewiseStaggeredGridView.countBuilder(
      {Key key,
        bool primary,
        ScrollController controller,
        EdgeInsetsGeometry padding,
  ...
  PagewiseStaggeredGridView.extentBuilder(
      {Key key,
        bool primary,
        ScrollController controller,
        EdgeInsetsGeometry padding,
